...
from qdvc.repo.query import FILTER_FILE_NAME
...
def commit(
    ...
    **kwargs: Any,
    ...
    if not repo.is_query_branch():
        raise Exception(
            "You tried to commit a query but you are not on a query branch. Please create a new query"
        )
    ...
    for file_diff in diff:
        ...
        if abs_path != filter_path:
            raise Exception(
                f"You tried to create a query branch by adding files others than {filter_path}. "
                f"Namely: {abs_path}"
            )
    ...
    if not found_filter_file:
        raise Exception(
            f"You need to modify and `git add {filter_path}` to create your query"
        )
    ...
    logger.info(
        "Successfuly commited query. Remember to push the branch and start using by `qdvc checkout`."
    )
    return message
